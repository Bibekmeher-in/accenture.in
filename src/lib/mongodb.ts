...
if (!process.env.MONGODB_URI) {
  clientPromise = Promise.reject(
    new Error('Please add your Mongo URI to .env')
  );
  // Attach no-op handler to avoid unhandled promise rejection warnings in environments where it is imported but not immediately awaited
  clientPromise.catch(() => { });
